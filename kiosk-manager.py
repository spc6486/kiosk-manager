...
def read_pi_splash_hidden():
    """Check if all Pi boot screens are suppressed:
    - EEPROM diagnostic (red/white screen, Pi 5 only)
    - GPU firmware rainbow splash
    - Kernel penguin logo
    """
    has_disable_splash = False
    has_logo_nologo = False
    has_diag_off = True  # default true if not Pi 5 / no EEPROM

    if CONFIG_TXT.exists():
        try:
            text = CONFIG_TXT.read_text()
            has_1 = bool(
                re.search(r'^disable_splash=1', text, re.MULTILINE))
            has_0 = bool(
                re.search(r'^disable_splash=0', text, re.MULTILINE))
            has_disable_splash = has_1 and not has_0
        except OSError:
            pass
    ...
    if CMDLINE_TXT.exists():
        try:
            text = CMDLINE_TXT.read_text()
            has_logo_nologo = "logo.nologo" in text
        except OSError:
            pass

    # Check EEPROM diagnostic and net install (Pi 5)
    if shutil.which("rpi-eeprom-config"):
        try:
            result = subprocess.run(
                ["sudo", "-n", "rpi-eeprom-config"],
                capture_output=True, text=True, timeout=5)
            eeprom_text = result.stdout
            # DISPLAY_DIAGNOSTIC defaults to 1 if absent
            if "DISPLAY_DIAGNOSTIC" in eeprom_text:
                has_diag_off = bool(
                    re.search(r'DISPLAY_DIAGNOSTIC=0', eeprom_text))
            else:
                has_diag_off = False
            # NET_INSTALL_AT_POWER_ON defaults to 1 if absent
            if "NET_INSTALL_AT_POWER_ON" in eeprom_text:
                has_net_off = bool(
                    re.search(r'NET_INSTALL_AT_POWER_ON=0', eeprom_text))
            else:
                has_net_off = True  # not present = not a problem
            has_diag_off = has_diag_off and has_net_off
        except (subprocess.CalledProcessError,
                subprocess.TimeoutExpired):
            pass

    return has_disable_splash and has_logo_nologo and has_diag_off


def set_pi_splash_hidden(hide):
    """Add or remove disable_splash, logo.nologo, and EEPROM diagnostic."""
    try:
        # config.txt: disable_splash
        if CONFIG_TXT.exists():
            text = CONFIG_TXT.read_text()
            cleaned = re.sub(
                r'^disable_splash=\d+\n?', '', text,
                flags=re.MULTILINE)
            if hide:
                new_text = "disable_splash=1\n" + cleaned
            else:
                new_text = cleaned
            if new_text != text:
                subprocess.run(
                    ["sudo", "-n", "tee", str(CONFIG_TXT)],
                    input=new_text.encode(), check=True,
                    stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
                    timeout=10)

        # cmdline.txt: logo.nologo
        if CMDLINE_TXT.exists():
            text = CMDLINE_TXT.read_text()
            has_it = "logo.nologo" in text
            if hide and not has_it:
                new_text = text.rstrip() + " logo.nologo\n"
                subprocess.run(
                    ["sudo", "-n", "tee", str(CMDLINE_TXT)],
                    input=new_text.encode(), check=True,
                    stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
                    timeout=10)
            elif not hide and has_it:
                new_text = text.replace(" logo.nologo", "").replace(
                    "logo.nologo ", "").replace("logo.nologo", "")
                subprocess.run(
                    ["sudo", "-n", "tee", str(CMDLINE_TXT)],
                    input=new_text.encode(), check=True,
                    stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
        ...
        # EEPROM: DISPLAY_DIAGNOSTIC + NET_INSTALL_AT_POWER_ON (Pi 5)
        if shutil.which("rpi-eeprom-config"):
            try:
                result = subprocess.run(
                    ["sudo", "-n", "rpi-eeprom-config"],
                ...
                # Handle DISPLAY_DIAGNOSTIC
                if "DISPLAY_DIAGNOSTIC" in eeprom_text:
                    eeprom_text = re.sub(
                        r'DISPLAY_DIAGNOSTIC=\d+',
                        'DISPLAY_DIAGNOSTIC=' + ('0' if hide else '1'),
                        eeprom_text)
                elif hide:
                    eeprom_text = eeprom_text.rstrip() + \
                        "\nDISPLAY_DIAGNOSTIC=0\n"

                # Handle NET_INSTALL_AT_POWER_ON
                if "NET_INSTALL_AT_POWER_ON" in eeprom_text:
                    eeprom_text = re.sub(
                        r'NET_INSTALL_AT_POWER_ON=\d+',
                        'NET_INSTALL_AT_POWER_ON=' + ('0' if hide else '1'),
                        eeprom_text)
                elif hide:
                    eeprom_text = eeprom_text.rstrip() + \
                ...
                os.unlink(tmp)
                    ...
                    subprocess.TimeoutExpired):
                pass  # Non-fatal — Pi 4 and earlier don't have this
...
class KioskManagerWindow(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        # Other
        section_other = Gtk.Label()
        section_other.set_markup("<b>Other</b>")
        section_other.set_xalign(0)
        boot_page.pack_start(section_other, False, False, 0)

        other_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        other_box.set_margin_start(12)

        self.shutdown_black_check = Gtk.CheckButton(
            label="Black screen on shutdown")
        self.shutdown_black_check.set_tooltip_text(
            "Show a black screen on shutdown instead of\n"
            "the boot image.")
        self.shutdown_black_check.set_active(read_shutdown_black())
        other_box.pack_start(self.shutdown_black_check, False, False, 0)

        self.pi_splash_check = Gtk.CheckButton(
            label="Hide Raspberry Pi boot screen")
        self.pi_splash_check.set_active(read_pi_splash_hidden())
        self.pi_splash_check.set_tooltip_text(
            "Suppress the EEPROM diagnostic, network install\n"
            "screen, rainbow splash, and kernel logo.\n"
            "Your boot image shows from the start.")
        other_box.pack_start(self.pi_splash_check, False, False, 0)

        self.tray_check = Gtk.CheckButton(label="Show tray icon")
        self.tray_check.set_active(is_tray_enabled())
        self.tray_check.set_tooltip_text(
            "Show a status icon in the taskbar for\n"
            "quick access to kiosk settings.")
        other_box.pack_start(self.tray_check, False, False, 0)

        boot_page.pack_start(other_box, False, False, 0)
    ...
    def _on_apply(self, _btn):
        ...
        want_hide_pi = self.pi_splash_check.get_active()
        pi_splash_changed = (want_hide_pi != read_pi_splash_hidden())
        ...
        if kiosk_enabled:
            app_bin = app_cmd.split()[0]
            if not os.path.exists(app_bin) and not shutil.which(app_bin):
                if not self._confirm(
                    "Application not found",
                    f"'{app_bin}' was not found.\n\n"
                    "Apply anyway?"):
                    return
        ...
        if pi_splash_changed:
            changes.append(
                f"Pi boot screen: "
                f"{'hidden' if want_hide_pi else 'shown'}")
        if tray_changed:
            changes.append(
        ...
        def _task():
            ...
            # Pi boot screen
            if pi_splash_changed:
                if not set_pi_splash_hidden(want_hide_pi):
                    errors.append("Failed to change Pi boot screen.")
