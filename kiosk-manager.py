...
def _find_swayidle_line():
    """Extract the swayidle line from the current user autostart, if any."""
    if not LABWC_USER_AUTOSTART.exists():
        return None
...
def _find_swayidle_from_system():
    """Check system autostart for swayidle as fallback."""
    if not LABWC_SYS_AUTOSTART.exists():
        return None
    for line in LABWC_SYS_AUTOSTART.read_text().splitlines():
        if "swayidle" in line:
            return line.strip()
    return None
...
def read_kiosk_state():
    ...
    state = {
        ...
        "hide_cursor": False,
        "swayidle_line": (_find_swayidle_line()
                          or _find_swayidle_from_system()
                          or "swayidle -w timeout 600 'wlopm --off \\*' "
                             "resume 'wlopm --on \\*' &"),
    }
...
def apply_kiosk(app_command, suppress_panel, suppress_desktop,
                enable, swayidle_line, auto_restart=False,
    ...
    and swayidle (additive, not a replacement).
    """
    ...
    if not enable:
        ...
        # Remove kiosk launcher from user autostart, keep swayidle
        if LABWC_USER_AUTOSTART.exists():
            _backup(LABWC_USER_AUTOSTART)
            text = LABWC_USER_AUTOSTART.read_text()
            keep = [l for l in text.splitlines()
                    if "kiosk-launch" not in l]
            # If only swayidle (or nothing useful) remains, keep it
            remaining = "\n".join(keep).strip()
            if remaining:
    ...
    # Add kiosk launcher to user autostart (alongside swayidle)
    _backup(LABWC_USER_AUTOSTART)

    lines = []
    # Preserve existing swayidle if present
    if LABWC_USER_AUTOSTART.exists():
        for line in LABWC_USER_AUTOSTART.read_text().splitlines():
            if "kiosk-launch" not in line and line.strip():
                lines.append(line.strip())

    # Add swayidle if not already present
    if swayidle_line and not any("swayidle" in l for l in lines):
        lines.append(swayidle_line)
...
def process_boot_sound(source_path):
        ...
        if has_sox:
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-f", "lavfi",
                 ...
                 "-t", "1.5", "-sample_fmt", "s16", str(warmup)],
                check=True, capture_output=True, timeout=10
            )
...
def apply_boot_sound(source_path):
    """Process and install a boot sound."""
    processed, error = process_boot_sound(source_path)
    if not processed:
        return False, error
...
class KioskManagerWindow(Gtk.Window):
    ...
    def _on_apply(self, _btn):
        ...
        summary = "\n".join(changes)
        if new_image or reprocess_image or new_sound:
        ...
        def _task():
            ...
            # New boot sound
            if new_sound:
                ok, err = apply_boot_sound(snd_path)
                if not ok:
