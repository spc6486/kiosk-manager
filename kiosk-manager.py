...
def read_pi_splash_hidden():
    """Check if the Pi rainbow splash and kernel logo are suppressed."""
    ...
    if CONFIG_TXT.exists():
        try:
            ...
            # Must have disable_splash=1 and NOT have disable_splash=0
            has_1 = bool(
    ...
    return has_disable_splash and has_logo_nologo


def set_pi_splash_hidden(hide):
    """Add or remove disable_splash=1 and logo.nologo."""
    try:
        # config.txt: disable_splash
        if CONFIG_TXT.exists():
            text = CONFIG_TXT.read_text()

            # Remove ALL existing disable_splash lines first
            cleaned = re.sub(
                r'^disable_splash=\d+\n?', '', text,
                flags=re.MULTILINE)

            if hide:
                # Insert at the top (global scope, before sections)
...
class KioskManagerWindow(Gtk.Window):
    ...
    def _build_ui(self):
        ...
        self.pi_splash_check.set_tooltip_text(
            "Suppress the rainbow splash and Pi logo.\n"
            "Your boot image shows from the start.")
        img_box.pack_start(self.pi_splash_check, False, False, 0)
