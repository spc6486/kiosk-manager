...
if [ "${1:-}" = "--uninstall" ]; then
    ...
    sudo rm -f "${AUTOSTART_SYS}.disabled"
    sudo rm -f "$SUDOERS"
...
# Pi boot screen
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/tee /boot/firmware/config.txt
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/tee /boot/firmware/cmdline.txt
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/rpi-eeprom-config
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/rpi-eeprom-config --apply *
# Tray autostart
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/mv /etc/xdg/autostart/kiosk-manager.desktop /etc/xdg/autostart/kiosk-manager.desktop.disabled
$USER_REAL ALL=(ALL) NOPASSWD: /usr/bin/mv /etc/xdg/autostart/kiosk-manager.desktop.disabled /etc/xdg/autostart/kiosk-manager.desktop
EOF
